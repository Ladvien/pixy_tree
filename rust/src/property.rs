...
pub enum PropertyValue {
    ...
    /// Curve mapping: maps input x ∈ [0,1] to output ∈ [y_min, y_max] with power curve
    Curve { y_min: f32, y_max: f32, power: f32 },
}

impl PropertyValue {
    /// Evaluate the property at position x ∈ [0,1]
    ///
    /// # Arguments
    /// * `x` - Position along parent branch (0.0 = base, 1.0 = tip)
    ...
    pub fn evaluate(&self, x: f32, rng: &mut SeededRng) -> f32 {
        match self {
            ...
            PropertyValue::Curve {
                ...
                let factor = x.clamp(0.0, 1.0).powf(*power);
...
/// Property evaluation mode selector
...
impl BranchProperty {
    ...
    pub fn evaluate(&self, x: f32, rng: &mut SeededRng) -> f32 {
        match self.mode {
            ...
            PropertyMode::Curve => {
                let factor = x.clamp(0.0, 1.0).powf(self.curve_power);
